      ...
      <div class="row">
        <div class="col-md-4 col-lg-5 col-xl-3 col-xs-12">
          <div class="form-inline">
            <a href="{{ route('pendapatsaranhukum.tambah') }}" class="btn btn-sm btn-primary"><i
              ...
              <tr>
                ...
                <td>
                  @if ($row->proses->count() > 0)
                    ...
                    {!! $row->proses[0]->pendapat_saran_proses_deskripsi !!}<br><small>{{ $row->proses[0]->operator . ', ' . \Carbon\Carbon::parse($row->proses[0]->created_at)->isoFormat('LL') }}</small>
                  @endif
                </td>
                <td class="pull-right">
                  <a href="/pendapatsaranhukum/proses/{{ $row->pendapat_saran_id }}" id='btn-del'
                    class='btn btn-success btn-xs m-r-3'><i class='fas fa-paper-plane'></i> Ganti Status</a><br><br>
                  <a href="/pendapatsaranhukum/edit/{{ $row->pendapat_saran_id }}" id='btn-del'
                    class='btn btn-grey btn-xs m-r-3'><i class='fas fa-edit'></i> Edit Data</a>
                  <a href="javascript:;"
                    onclick="hapus('{{ $row->pendapat_saran_id }}', '{{ $row->pendapat_saran_laporan_nomor }}')"
                    id='btn-del' class='btn btn-danger btn-xs'><i class='fas fa-trash'></i> Hapus</a>
                </td>
              </tr>
            @endforeach
          </tbody>
