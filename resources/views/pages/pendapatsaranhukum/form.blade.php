    ...
    <form action="{{ route('pendapatsaranhukum.' . strtolower($aksi)) }}" method="post" data-parsley-validate="true"
      ...
      <div class="panel-body">
        ...
        <div class="form-group">
          ...
          <input class="form-control" type="text" name="pendapat_saran_laporan_nomor"
            value="{{ $aksi == 'Edit' ? $data->pendapat_saran_laporan_nomor : old('pendapat_saran_laporan_nomor') }}"
            required data-parsley-minlength="1" data-parsley-maxlength="250" autocomplete="off" />
        </div>
        <div class="form-group">
          <label class="control-label">Tanggal</label>
          <input type="text" readonly required class="form-control datepicker" name="pendapat_saran_tanggal"
            value="{{ date('d F Y', strtotime($aksi == 'Edit' ? $data->pendapat_saran_tanggal : (old('pendapat_saran_tanggal') ? old('pendapat_saran_tanggal') : now()))) }}" />
        </div>
        <div class="form-group">
          <label class="control-label">Judul</label>
          <input class="form-control" type="text" name="pendapat_saran_judul"
            value="{{ $aksi == 'Edit' ? $data->pendapat_saran_judul : old('pendapat_saran_judul') }}" required
            data-parsley-minlength="1" data-parsley-maxlength="250" autocomplete="off" />
        </div>
        <div class="form-group">
          <label class="control-label">Satuan Kerja</label>
          <select class="form-control selectpicker" name="pendapat_saran_satuan_kerja" data-live-search="true"
            data-style="btn-info" data-width="100%">
            @foreach ($satuanKerja as $row)
              <option value="{{ $row }}" {{ old('pendapat_saran_satuan_kerja') == $row ? 'selected' : '' }}>
                {{ $row }}</option>
            @endforeach
          </select>
        </div>
        ...
        @if ($aksi == 'Tambah')
          ...
          <div class="note note-secondary m-b-15">
            ...
            <div class="form-group">
              ...
              <select class="form-control selectpicker" style="width : 100%" name="pendapat_saran_proses_status"
                id="pendapat_saran_proses_status" data-style="btn-info" data-width="100%">
            ...
            <div class="form-group">
              <label class="control-label">Tanggal</label>
              <input type="text" readonly required class="form-control datepicker" name="pendapat_saran_proses_tanggal"
                value="{{ date('d F Y', strtotime($aksi == 'Edit' ? $data->pendapat_saran_proses_tanggal : (old('pendapat_saran_proses_tanggal') ? old('pendapat_saran_proses_tanggal') : now()))) }}" />
            </div>
            <div class="form-group">
