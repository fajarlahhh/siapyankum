      ...
      <div class="row">
        <div class="col-md-4 col-lg-5 col-xl-3 col-xs-12">
          <div class="form-inline">
            <a href="{{ route('praperadilan.tambah') }}" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i>
              Tambah</a>
          </div>
        </div>
              ...
              <tr>
                ...
                <td>
                  @if ($row->proses->count() > 0)
                    ...
                    {!! $row->proses[0]->bantuan_hukum_proses_deskripsi !!}<br><small>{{ $row->proses[0]->operator . ', ' . \Carbon\Carbon::parse($row->proses[0]->created_at)->isoFormat('LL') }}</small>
                  @endif
                </td>
                <td class="pull-right">
                  <a href="/praperadilan/proses/{{ $row->bantuan_hukum_id }}" id='btn-del'
                    class='btn btn-success btn-xs m-r-3'><i class='fas fa-paper-plane'></i> Ganti Status</a><br><br>
                  <a href="/praperadilan/edit/{{ $row->bantuan_hukum_id }}" id='btn-del'
                    class='btn btn-grey btn-xs m-r-3'><i class='fas fa-edit'></i> Edit Data</a>
                  <a href="javascript:;"
                    onclick="hapus('{{ $row->bantuan_hukum_id }}', '{{ $row->bantuan_hukum_laporan_nomor }}')"
