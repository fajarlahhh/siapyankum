      ...
      <div class="panel-body">
        ...
        <div class="form-group">
          ...
          <input class="form-control" type="text" name="bantuan_hukum_laporan_nomor"
            value="{{ $aksi == 'Edit' ? $data->bantuan_hukum_laporan_nomor : old('bantuan_hukum_laporan_nomor') }}"
            required data-parsley-minlength="1" data-parsley-maxlength="250" autocomplete="off" />
        </div>
        <div class="form-group">
          <label class="control-label">Tanggal</label>
          <input type="text" readonly required class="form-control datepicker" name="bantuan_hukum_tanggal"
            placeholder="Tgl. Mulai"
            value="{{ date('d F Y', strtotime($aksi == 'Edit' ? $data->bantuan_hukum_tanggal : (old('bantuan_hukum_tanggal') ? old('bantuan_hukum_tanggal') : now()))) }}" />
        </div>
        <div class="form-group">
          <label class="control-label">Judul</label>
          <input class="form-control" type="text" name="bantuan_hukum_judul"
            value="{{ $aksi == 'Edit' ? $data->bantuan_hukum_judul : old('bantuan_hukum_judul') }}" required
            data-parsley-minlength="1" data-parsley-maxlength="250" autocomplete="off" />
        </div>
        <div class="form-group">
          <label class="control-label">Satuan Kerja</label>
          <select class="form-control selectpicker" name="bantuan_hukum_satuan_kerja" data-live-search="true"
            data-style="btn-info" data-width="100%">
            @foreach ($satuanKerja as $row)
              <option value="{{ $row }}" {{ old('bantuan_hukum_satuan_kerja') == $row ? 'selected' : '' }}>
                {{ $row }}</option>
            @endforeach
          </select>
        </div>
        ...
        @if ($aksi == 'Tambah')
          ...
          <div class="note note-secondary m-b-15">
            ...
            <div class="form-group">
              <label class="control-label">Tanggal</label>
              <input type="text" readonly required class="form-control datepicker" name="bantuan_hukum_proses_tanggal"
