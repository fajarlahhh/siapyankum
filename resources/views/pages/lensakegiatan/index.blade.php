        ...
        <div class="col-md-4 col-lg-5 col-xl-3 col-xs-12">
          @role('user|administrator')
            <div class="form-inline">
              <a href="{{ route('datalensakegiatan.tambah') }}" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i>
                Tambah</a>
            </div>
          @endrole
        </div>
          ...
          <tbody>
            @foreach ($data as $index => $row)
              <tr>
                ...
                <td>{{ $row->lensa_kegiatan_satuan_kerja }}</td>
                <td><a href="{{ url($row->lensa_kegiatan_file) }}" target="_blank" class="btn btn-xs btn-success">Buka
                    File</a></td>
                <td>{{ $row->operator }}</td>
                ...
                <td>
                  @role('user|administrator')
                    <a href="javascript:;"
                      onclick="hapus('{{ $row->lensa_kegiatan_id }}', '{{ $row->lensa_kegiatan_judul }}')"
