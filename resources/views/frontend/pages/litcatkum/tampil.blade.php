...
@php
$warna = ['grey', 'purple', 'indigo', 'primary', 'info', 'warning', 'pink', 'danger', 'success', 'green', 'lime'];
@endphp

@section('content')
  <h5 class="text-center text-light"><small class=" text-light">Penelitian Catatan
      Hukum</small><br>{{ $data->litcatkum_nama }}</h5>
  <br>
  <div class="note note-success m-b-15" style="padding: 0px !important; ">
    <table class="table">
      <tbody>
        <tr>
          <th class="width-150">Nama</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_nama }}</td>
        </tr>
        <tr>
          <th>Pangkat</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_pangkat }}</td>

        </tr>
        <tr>
          <th>No. NRP</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_nrp }}</td>

        </tr>
        <tr>
          <th>Jabatan</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_jabatan }}</td>

        </tr>
        <tr>
          <th>Satuan Kerja</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_satuan_kerja }}</td>

        </tr>
        <tr>
          <th>No. LP</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_nomor_lp }}</td>

        </tr>
        <tr>
          <th>Tahun LP</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_tahun_lp }}</td>

        </tr>
        <tr>
          <th>Jenis Pelanggaran</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_jenis_pelanggaran }}</td>

        </tr>
        <tr>
          <th>No. PSH</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_nomor_psh }}</td>

        </tr>
        <tr>
          <th>No. SKHD</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_nomor_skhd }}</td>

        </tr>
        <tr>
          <th>Hukuman</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_hukuman }}</td>

        </tr>
        <tr>
          <th>No. PRS</th>
          <td width="5">:</td>
          <td>{{ $data->litcatkum_nomor_prs }}</td>
  ...
  <script type="text/javascript">
    var pdf = '{{ url($data->lensa_kegiatan_file) }}';
        ...
        <div id="toolbarContainer">
          <div id="toolbarViewer">
            <div id="toolbarViewerLeft">
              <button class="hidden" id="sidebarToggle" class="toolbarButton" title="Toggle Sidebar" tabindex="11"
                data-l10n-id="toggle_sidebar">
                <span data-l10n-id="toggle_sidebar_label">Toggle Sidebar</span>
              </button>
              <button class="hidden" id="viewFind" class="toolbarButton" title="Find in Document" tabindex="12"
                data-l10n-id="findbar">
                <span data-l10n-id="findbar_label">Find</span>
              </button>
              <div class="splitToolbarButton hiddenSmallView">
                <a class="toolbarButton pageUp" title="Previous Page" id="previous" tabindex="13"
                  data-l10n-id="previous">
                  <span data-l10n-id="previous_label">Previous</span>
                </a>
                <div class="splitToolbarButtonSeparator"></div>
                <a class="toolbarButton pageDown" title="Next Page" id="next" tabindex="14"
                  data-l10n-id="next">
            ...
            <div id="toolbarViewerRight">
              <a id="presentationMode" class="toolbarButton presentationMode hiddenLargeView"
                title="Switch to Presentation Mode" tabindex="31" data-l10n-id="presentation_mode">
                <span data-l10n-id="presentation_mode_label">Presentation Mode</span>
              </a>

              <button class="hidden" id="openFile" class="toolbarButton openFile hiddenLargeView" title="Open File"
                tabindex="32" data-l10n-id="open_file">
                <span data-l10n-id="open_file_label">Open</span>
              </button>

              <button class="hidden" id="print" class="toolbarButton print hiddenMediumView" title="Print"
                tabindex="33" data-l10n-id="print">
                <span data-l10n-id="print_label">Print</span>
              </button>

              <button class="hidden" id="download" class="toolbarButton download hiddenMediumView" title="Download"
                tabindex="34" data-l10n-id="download">
                <span data-l10n-id="download_label">Download</span>
              </button>
              <a href="#" class="hidden" id="viewBookmark" class="toolbarButton bookmark hiddenSmallView"
              ...
              <button class="hidden" id="secondaryToolbarToggle" class="toolbarButton" title="Tools" tabindex="36"
                data-l10n-id="tools">
            ...
            <div id="toolbarViewerMiddle">
              <div class="splitToolbarButton">
                <a id="zoomOut" class="toolbarButton zoomOut" title="Zoom Out" tabindex="21"
  ...
  <div class="text-center">
    <a href="{{ '/frontend/lensakegiatan/download/' . $data->lensa_kegiatan_id }}"
      class="text-center btn btn-xs btn-success">Download</a>
